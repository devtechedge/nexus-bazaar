      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs text-left space-y-5">
            <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
              ...
              <h3 className="text-xs font-black uppercase tracking-wider text-slate-800">My Privacy Choices</h3>
            </div>

            <p className="text-[11px] text-slate-500 leading-relaxed">
              Decide how we handle your data. Turn settings on or off with simple toggles to protect your identity.
            </p>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start justify-between gap-4 p-3.5 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-all">
                <div className="space-y-0.5 text-left">
                  <span className="text-xs font-bold text-slate-800 block flex items-center gap-2">
                    Make my shopping experience personal
                    <span className={`text-[8px] font-mono font-bold px-1.5 py-0.2 rounded ${behavioralTracking ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>
                      {behavioralTracking ? 'Tailored to Me' : 'Standard View'}
                    </span>
                  </span>
                  <span className="text-[10px] text-slate-400 block leading-normal">
                    When turned on, we'll show you recommended items based on what you look at. If turned off, you will see a general storefront.
                  </span>
                </div>
                <button
                  type="button"
              ...
              {/* Performance Telemetry: HIDDEN as per "hide telemetry" requirement, but state is kept internally */}

              {/* 75. Cryptographic Review Pseudonym Mask */}
              <div className="flex items-start justify-between gap-4 p-3.5 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-all">
                <div className="space-y-0.5 text-left">
                  <span className="text-xs font-bold text-slate-800 block flex items-center gap-1.5">
                    <span>Hide my real name on the store</span>
                    {pseudonymActive && (
                  ...
                  <span className="text-[10px] text-slate-400 block leading-normal">
                    Use a random nickname (like Anonym_Buyer_d7a2f) instead of your real name on reviews, comments, and public posts to protect your identity.
                  </span>
                </div>
                <button
                  type="button"
              ...
              {pseudonymActive && (
                <div className="bg-emerald-50 border border-emerald-150 rounded-xl p-3 text-[10px] font-mono text-emerald-800 space-y-1 leading-normal">
                  <span className="font-bold uppercase block">Your Secret Nickname:</span>
                  <div>Original Name: <strong className="text-emerald-950 font-bold">{currentUser.name}</strong></div>
                  <div>Assigned Nickname: <strong className="text-emerald-900 bg-emerald-100 px-1 rounded">Anonym_Buyer_{currentUser.id.substring(5, 10) || 'd7a2f'}</strong></div>
                  <span className="text-[9px] text-emerald-600 block pt-1 leading-normal">
                    🛡️ This safe nickname will show on your product reviews and public posts automatically!
                  </span>
                </div>
              )}
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs text-left space-y-5">
            <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
              <span className="text-lg">💳</span>
              <h3 className="text-xs font-black uppercase tracking-wider text-slate-800">Temporary Shopping Cards</h3>
            </div>

            <p className="text-[11px] text-slate-500 leading-relaxed">
              Create a temporary card number to protect your real credit card. Set a spend limit and the card will stop working after you buy your items, keeping your banking info 100% safe.
            </p>

            {/* Create Card Form */}
            <form onSubmit={handleCreateVirtualCard} className="bg-slate-50 border border-slate-150 rounded-xl p-4 space-y-4">
              <span className="text-[10px] font-black uppercase tracking-wider text-slate-500 block">Create Temporary Shopping Card</span>
              
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <label className="text-[9px] font-mono text-slate-400 uppercase block mb-1">Card Nickname</label>
                  <input
                    type="text"
                    required
                    value={newCardLabel}
                ...
                <div>
                  <label className="text-[9px] font-mono text-slate-400 uppercase block mb-1">Spending Limit ($)</label>
                  <input
                    type="number"
                    min="10"
                    max="1000"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs text-left space-y-4">
            <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
              <span className="text-lg">🧬</span>
              <h3 className="text-xs font-black uppercase tracking-wider text-slate-800">Secure Device Check</h3>
            </div>

            <p className="text-[11px] text-slate-500 leading-relaxed">
              We verify that your browser and location match your usual habits to keep your account safe from hackers.
            </p>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs text-left space-y-4">
            <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
              ...
              <h3 className="text-xs font-black uppercase tracking-wider text-slate-800">Anti-Bot Safety Shield</h3>
            </div>

            <p className="text-[11px] text-slate-500 leading-relaxed">
              To keep our store fast and prevent automated bots from overloading our system, we limit search speeds.
            </p>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs text-left space-y-4">
            <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
              <span className="text-lg">🧹</span>
              <h3 className="text-xs font-black uppercase tracking-wider text-slate-800">Auto-Cleanup for Carts</h3>
            </div>

            <p className="text-[11px] text-slate-500 leading-relaxed">
              We automatically clear out abandoned shopping baskets after 3 days to keep our system running light and fast.
            </p>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs text-left space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-3 gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-xs font-black uppercase tracking-wider text-slate-800">My Account Security History</h3>
              <p className="text-[10px] text-slate-400 leading-none mt-1">Check this history list to see a record of when security settings were updated or changed.</p>
            </div>
          </div>
